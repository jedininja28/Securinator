...
:Sm2
TYPE "%RootDir%"scans\mm.txt || IF ERRORLEVEL = 1 ECHO Creating File... && WMIC MEMORYCHIP > "%RootDir%"scans\mm.txt && TYPE "%RootDir%"scans\mm.txt
...
:Sm3
TYPE "%RootDir%"scans\ip.txt || IF ERRORLEVEL = 1 ECHO Creating File... && ipconfig /allcompartments /all > "%RootDir%"scans\ip.txt && TYPE "%RootDir%"scans\ip.txt
PAUSE
GOTO :scans

:: TYPE "%RootDir%"scans\ip_%date:~0,3%%date:~-7,2%%date:~-4,4%.txt
:: IF ERRORLEVEL = 1 ECHO Creating File... && ipconfig /allcompartments /all > "%RootDir%"scans\ip_%date:~0,3%%date:~-7,2%%date:~-4,4%.txt && TYPE "%RootDir%"scans\ip_%date:~0,3%%date:~-7,2%%date:~-4,4%.txt

PAUSE
GOTO :scans

:Sm4
TYPE "%RootDir%"scans\ns.txt || IF ERRORLEVEL = 1 ECHO Creating File... && netstat -a -b -r -s -t > "%RootDir%"scans\ns.txt | TYPE "%RootDir%"scans\ns.txt
PAUSE
GOTO :scans

:Sm5
CALL "%RootDir%"scans\tr.txt || IF ERRORLEVEL = 1 ECHO Creating File... && Tree /F > "%RootDir%"scans\tr.txt && CALL "%RootDir%"scans\tr.txt
PAUSE
GOTO :scans

:Sm6
TYPE "%RootDir%"scans\tl.txt || IF ERRORLEVEL = 1 ECHO Creating File... && tasklist /SVC > "%RootDir%"scans\tl.txt && TYPE "%RootDir%"scans\tl.txt
PAUSE
GOTO :scans

:Sm7
TYPE "%RootDir%"scans\dq.txt || IF ERRORLEVEL = 1 ECHO Creating File... && driverquery /V > "%RootDir%"scans\dq.txt && TYPE "%RootDir%"scans\dq.txt )
PAUSE
GOTO :scans

:Sm8
TYPE "%RootDir%"scans\st.txt || IF ERRORLEVEL = 1 ECHO Creating File... && schtasks /query > "%RootDir%"scans\st.txt && TYPE "%RootDir%"scans\st.txt
PAUSE
GOTO :scans

:Sm9
TYPE "%RootDir%"scans\at.txt || IF ERRORLEVEL = 1 ECHO Creating File... && attrib /s > "%RootDir%"scans\at.txt && TYPE "%RootDir%"scans\at.txt
PAUSE
GOTO :scans

:Sm10
TYPE "%RootDir%"scans\cu.txt || IF ERRORLEVEL = 1 ECHO Creating File... && wmic cpu > "%RootDir%"scans\cu.txt && TYPE "%RootDir%"scans\cu.txt
PAUSE
GOTO :scans

:Sm11
TYPE "%RootDir%"scans\pt.txt || IF ERRORLEVEL = 1 ECHO Creating File... && wmic port > "%RootDir%"scans\pt.txt && TYPE "%RootDir%"scans\pt.txt
PAUSE
GOTO :scans

:Sm12
TYPE "%RootDir%"scans\bs.txt || IF ERRORLEVEL = 1 ECHO Creating File... && wmic bios > "%RootDir%"scans\bs.txt && TYPE "%RootDir%"scans\bs.txt
PAUSE
GOTO :scans
